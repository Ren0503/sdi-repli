...
Thuật toán băm nhất quán được giới thiệu bởi Karger tại MIT [1] gồm hai bươc cơ bản sau:
- Ánh xạ server và khoá lên vòng bằng cách sự dụng hàm băm phân phối đồng nhất.
...
Một virtual node - nút ảo đề cập đến một nút thật, và mỗi server được biểu diễn bằng nhiều virutal node trên vòng. Hình 5-12, cả server 0 và 1 đều có 3 virtual node. Con số 3 ở đây được chọn tùy ý; và trong các hệ thống thế giới thực số lượng các virtual node lớn hơn nhiều.
...
Để tìm server mà khoá được lưu, ta đi theo chiều kim đồng hồ từ vị trí khoá và tìm đến virtual node đầu tiên trên vòng. Trong hình 5-13, để tìm server lưu khoá k0, ta đi từ vị trí k0 theo chiều kim đồng hồ đến virtual node là s1_1, tức là server 1.

![](./assets/virtual2.png)

Khi số lượng các virtual node tăng lên, sự phân bố các khóa trở nên cân bằng hơn.
Điều này là do độ lệch chuẩn nhỏ dần với nhiều virtual node hơn, dẫn đến phân phối dữ liệu cân bằng. Độ lệch chuẩn được đo lường các khảo sát dữ liệu. Kết quả của một thử nghiệm được thực hiện bởi nghiên cứu trực tuyến [2] cho thấy rằng với một hoặc hai trăm virtual node, độ lệch chuẩn sẽ nằm giữa 5% (200 virtual node) và 10% (100 virtual node) của giá trị trung bình. Độ lệch chuẩn sẽ nhỏ hơn khi chúng ta tăng số lượng virtual node. Tuy nhiên, điều này sẽ cần nhiều không gian hơn để lưu trữ dữ liệu về các virtual node.

Đây là một sự cân bằng và chúng ta có thể điều chỉnh số lượng virtual node để phù hợp với yêu cầu hệ thống.
...
Trong chương này, chúng ta đã có một cuộc thảo luận chuyên sâu về băm nhất quán, bao gồm lý do tại sao nó cần và cách thức hoạt động của nó. Các lợi ích của băm nhất quán bao gồm:
